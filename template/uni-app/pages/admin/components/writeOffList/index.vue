	...
	<view>
		<view class='shoppingCart copy-data'>
								...
								<view class='picTxt acea-row row-between-wrapper'>
									...
									<view class='text'>
										<view class="title">
											...
											<view v-if="item.is_writeoff == 1" class="txt">Viết tắt</view>
											<view v-if="item.is_writeoff == 0 && item.surplus_num == item.cart_num" class="txt bluecol">Không được viết tắt</view>
											<view v-if="item.is_writeoff == 0 && item.surplus_num != item.cart_num" class="txt orangcol">
												Viết tắt{{item.cart_num - item.surplus_num}}miếng</view>
										</view>
			...
			<view class='footer acea-row row-between-wrapper'>
				...
				<view>
					<button class='money' type="primary" @click="verification">{{checked?'Một cú nhấp chuột':'xác nhận'}}Viết tắt({{numChecked}})</button>
				</view>
			</view>
		</view>
		<view v-if="box">
			<view class="box">
				<view class="small_box">
					...
					<view class="acea-row btn-box">
						<view v-if="lets == 1 && list.total_num == parseInt(list.writeoff_count)+writeOffNum" class="btn" @click="ok(1)">biết</view>
						<navigator v-if="lets > 1 && list.total_num != parseInt(list.writeoff_count)+writeOffNum" :url='"/pages/admin/distribution/scanning/index?code="+attr.code' hover-class='none'
							open-type="redirect" class="btn">Quay lại danh sách</navigator>
						<navigator v-if="(lets > 1 && list.total_num == parseInt(list.writeoff_count)+writeOffNum)||(lets==1&&list.total_num != parseInt(list.writeoff_count)+writeOffNum)"
							url="/pages/admin/distribution/index" hover-class='none' open-type="redirect" class="btn">Trở về trang chủ</navigator>
						<view v-if="list.total_num != parseInt(list.writeoff_count)+writeOffNum" class="btn on" @click="ok(0)">Tiếp tục viết tắt</view>
						<navigator v-if="lets > 1 && list.total_num == parseInt(list.writeoff_count)+writeOffNum" :url='"/pages/admin/distribution/scanning/index?code="+attr.code' open-type="redirect"
							hover-class='none' class="btn">Xóa các đơn đặt hàng khác</navigator>
					</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		data() {
			return {
				...
				attr: { //Chuyển đổi giá trị thành phần
					cartAttr: false,
					id: [],
					code: '',
					type: 0
