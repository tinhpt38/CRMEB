			...
			<view class='evaluate-con'>
				...
				<view class='score'>
					<view class='item acea-row row-middle' v-for="(item,indexw) in scoreList" :key="indexw">
						...
						<text class='evaluate'>{{item.index === -1 ? "" : item.index + 1 + $t(`ngôi sao`)}}</text>
					</view>
					<view class='textarea'>
						<textarea :placeholder='$t(`Sản phẩm có đáp ứng mong đợi của bạn không? Hãy cho chúng tôi biết suy nghĩ của bạn và chia sẻ nó với những người muốn mua nó.`)' name="comment"
					...
					<button class='evaluateBnt bg-color' formType="submit">{{$t(`Đánh giá ngay bây giờ`)}}</button>
				</view>
			</view>
		</form>
		<!-- #ifdef MP -->
	...
	export default {
		...
		data() {
			return {
				pics: [],
				scoreList: [{
						name: this.$t(`Chất lượng sản phẩm`),
						stars: ["", "", "", "", ""],
						index: -1
					},
					{
						name: this.$t(`thái độ phục vụ`),
						stars: ["", "", "", "", ""],
						index: -1
					}
				],
		...
		methods: {
			...
			/**
			 * Nhận thông tin chi tiết của một sản phẩm
			 * 
			 */
			...
			formSubmit(e) {
				...
				if (!value.comment) return that.$util.Tips({
					title: that.$t(`Hãy điền suy nghĩ của bạn về em bé`)
				});
				...
				orderComment(value).then(res => {
					...
					if (res.data.to_lottery) {
						...
						that.$util.Tips({
							title: that.$t(`Cảm ơn bạn đã đánh giá của bạn`),
							icon: 'success'
						}, jumpPath);
					} else {
						that.$util.Tips({
