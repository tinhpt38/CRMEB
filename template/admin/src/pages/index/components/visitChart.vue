          ...
          <div>
            <el-row justify="center" align="middle">
              <el-col :span="8" class="card-title">
                ...
                ></el-avatar>
                <h4 class="ivu-pl-8">{{ $t('message.pages.index.visitChart.order') }}</h4>
              </el-col>
              <el-col :span="16" class="ivu-text-right">
                <el-radio-group v-model="visitDate" type="button" class="ivu-mr-8" @input="handleChangeVisitType">
                  <el-radio-button label="thirtyday">{{ $t('message.pages.index.visitChart.day30') }}</el-radio-button>
                  <el-radio-button label="week">{{ $t('message.pages.index.visitChart.week') }}</el-radio-button>
                  <el-radio-button label="month">{{ $t('message.pages.index.visitChart.month') }}</el-radio-button>
                  <el-radio-button label="year">{{ $t('message.pages.index.visitChart.year') }}</el-radio-button>
                </el-radio-group>
              </el-col>
            </el-row>
          </div>
        ...
        .then(async (res) => {
          ...
          (this.series = this.infoList.series || []),
            (this.yAxisData = [
              {
                type: 'value',
                name: this.$t('message.pages.index.visitChart.amount'),
                axisLine: {
                  show: false,
                },
                axisTick: {
