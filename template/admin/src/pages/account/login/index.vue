        ...
        <el-form ref="formInline" :model="formInline" :rules="ruleInline" @keyup.enter="handleSubmit('formInline')">
          <el-form-item prop="username">
            <el-input
              ...
              :placeholder="$t('message.login.usernamePlaceholder')"
              size="large"
            />
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      ruleInline: {
        username: [{ required: true, message: this.$t('message.login.usernameRequired'), trigger: 'blur' }],
        password: [{ required: true, message: this.$t('message.login.passwordRequired'), trigger: 'blur' }],
      },
      login_captcha: 0,
      login_logo: '',
      swiperList: [],
  ...
  methods: {
    swiperData() {
      loginInfoApi()
        .then((res) => {
          const data = res.data || {};
          document.title = `${data.site_name} - ${this.$t('message.login.pageTitle')}`;
      ...
      AccountLogin({
        ...
        .then(async (res) => {
          ...
          try {
            if (data.queue === false) {
              this.$notify.warning({
                title: this.$t('message.login.noticeTitle'),
                dangerouslyUseHTMLString: true,
                message: this.$t('message.login.queueNoticeMessage'),
                duration: 30000,
              });
            }
            if (data.timer === false) {
              setTimeout(() => {
                this.$notify.warning({
                  title: this.$t('message.login.noticeTitle'),
                  dangerouslyUseHTMLString: true,
                  message: this.$t('message.login.timerNoticeMessage'),
                  duration: 30000,
                });
              }, 0);
            }
      ...
      getWorkermanUrl().then((res) => {
        ...
        socket.onerror = socket.onclose = () => {
          if (!isNotice) {
            ...
            this.$notify.warning({
              title: this.$t('message.login.noticeTitle'),
              message: this.$t('message.login.socketNoticeMessage'),
              dangerouslyUseHTMLString: true,
              duration: 30000,
            });
          }
    ...
    closefail() {
      this.$message.error(this.$t('message.login.verifyFailed'));
    },
    handleResize() {
