<template>
  <div>
    <el-card :bordered="false" shadow="never" class="ivu-mt" :body-style="{ padding: 0 }">
      <div class="padding-add">
        <el-form
          ...
          inline
        >
          <el-form-item :label="$t('message.productParam.templateSearch') + '：'">
            <el-input
              clearable
              v-model="artFrom.name"
              :placeholder="$t('message.productParam.templateNamePlaceholder')"
    ...
    <el-card :bordered="false" shadow="never" class="ivu-mt mt16">
      <el-button type="primary" v-db-click @click="paramAdd">{{ $t('message.productParam.addProductParam') }}</el-button>
      <el-table
        ref="table"
        :data="tableList"
        v-loading="loading"
      ...
      >
        ...
        <el-table-column :label="$t('message.productParam.templateName')" min-width="130">
          <template slot-scope="scope">
            <span>{{ scope.row.name }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('message.productParam.createTime')" min-width="130">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('message.productList.operation')" fixed="right" width="120">
          <template slot-scope="scope">
            ...
            <a v-db-click @click="del(scope.row, $t('message.productParam.deleteParamConfirm'), scope.$index)">{{ $t('message.productCommon.delete') }}</a>
          </template>
        </el-table-column>
      </el-table>
