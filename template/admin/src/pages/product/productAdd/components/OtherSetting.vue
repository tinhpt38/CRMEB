<template>
  ...
  <el-row>
    <el-col :span="24">
      <el-form-item :label="$t('message.productAdd.productKeyword') + '：'">
        <el-input
          class="content_width"
          v-model.trim="formValidate.keyword"
          :placeholder="$t('message.productAdd.productKeywordPlaceholder')"
          maxlength="100"
          show-word-limit
        />
        <div class="tips-info">{{ $t('message.productAdd.productKeywordTip') }}</div>
      </el-form-item>
    </el-col>
    <el-col :span="24">
      <el-form-item :label="$t('message.productAdd.productIntro') + '：'">
        <el-input
          class="content_width"
          v-model.trim="formValidate.store_info"
          type="textarea"
          :rows="3"
          :placeholder="$t('message.productAdd.productIntroPlaceholder')"
          maxlength="100"
          show-word-limit
        />
        <div class="tips-info">{{ $t('message.productAdd.productIntroTip') }}</div>
      </el-form-item>
    </el-col>
    <el-col :span="24">
      <el-form-item :label="$t('message.productAdd.productCommand') + '：'">
        <el-input
          v-model.trim="formValidate.command_word"
          :placeholder="$t('message.productAdd.productCommandPlaceholder')"
          type="textarea"
          :rows="3"
          class="content_width"
        />
        <div class="tips-info">{{ $t('message.productAdd.productCommandTip') }}</div>
      </el-form-item>
    </el-col>

    <el-col :span="24">
      <el-form-item :label="$t('message.productAdd.productRecommendImage') + '：'">
        <div class="pictrueBox" v-db-click @click="modalPicTap('dan', 'recommend_image')">
          ...
          <div class="tips-info">{{ $t('message.productAdd.productRecommendImageTip') }}</div>
        </div>
      </el-form-item>
    </el-col>
    <el-col :span="24">
      <el-form-item :label="$t('message.productAdd.serviceProtection') + '：'">
        ...
        <el-button v-else type="primary" v-db-click @click="addProtection">{{ $t('message.productAdd.addProtection') }}</el-button>
        <div class="tips-info">{{ $t('message.productAdd.serviceProtectionTip') }}</div>
      </el-form-item>
    </el-col>
    <el-col :span="24">
      <el-form-item :label="$t('message.productAdd.productParams') + '：'">
        <el-select
          v-model="paramsType"
          clearable
    ...
    <el-col :span="24">
      <el-form-item :label="$t('message.productAdd.customForm') + '：'">
        ...
        <div class="addCustom_content" v-if="customBtn">
          <div v-for="(item, index) in formValidate.custom_form" :key="index" class="custom_box">
            <el-input
              ...
              :placeholder="$t('message.productAdd.formTitle') + (index + 1)"
            ...
            <el-checkbox v-model="item.status">{{ $t('message.productAdd.required') }}</el-checkbox>
            <div class="addfont" v-db-click @click="delcustom(index)">{{ $t('message.productCommon.delete') }}</div>
          </div>
        </div>
        <div class="addCustomBox" v-show="customBtn">
          <div class="btn" v-db-click @click="addcustom">+ {{ $t('message.productAdd.addForm') }}</div>
          <div class="tips-info">{{ $t('message.productAdd.customFormTip') }}</div>
        </div>
      </el-form-item>
    </el-col>
  </el-row>
