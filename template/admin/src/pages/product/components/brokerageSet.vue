<template>
  <div>
    <el-dialog :visible.sync="visible" :title="$t('message.productList.customCommission')" width="700">
      <el-form :model="formData" label-width="80px">
        <el-form-item :label="$t('message.productList.rebateSettings')">
          <el-radio-group v-model="formData.is_sub" @input="changeSubType">
            <el-radio :label="0">{{ $t('message.productList.defaultRatio') }}</el-radio>
            <el-radio :label="1">{{ $t('message.productList.customCommission2') }}</el-radio>
          </el-radio-group>
          <div class="fs-12 tips-info" v-show="formData.is_sub">
            {{ $t('message.productList.switchToDefaultRatio') }}
          </div>
        </el-form-item>
        <el-form-item label-width="0">
          <el-table size="small" border max-height="460" :data="attrData" style="width: 100%">
            <el-table-column prop="pic" :label="$t('message.productList.specImage')" min-width="90" align="center">
            ...
            <el-table-column prop="suk" :label="$t('message.productList.productSpec5')" min-width="120" align="center"></el-table-column>
            <el-table-column prop="price" :label="$t('message.productList.sellingPrice')" min-width="120" align="center"></el-table-column>
            <el-table-column min-width="120" align="center">
              <template slot="header" slot-scope="scope">
                <span>{{ $t('message.productList.firstLevelRebate') }}</span>
                <el-popover
                  ref="popoverRef_one"
                  placement="top"
                  width="280"
                ...
                >
                  <div class="pop-title">{{ $t('message.productList.batchSetFirstLevelRebate') }}</div>
                  <div class="mt-14">
                    <el-radio-group v-model="brokerageSetType">
                      <el-radio :label="0">{{ $t('message.productList.specifiedPrice') }}</el-radio>
                      <el-radio :label="1">{{ $t('message.productList.ratio2') }}</el-radio>
                    </el-radio-group>
                  </div>
                  <div class="mt10 mb10 acea-row">
                    <el-input type="number" class="popover-input" v-model="brokerage" @input="brokerageReplace">
                      <template slot="suffix">
                        <span>{{ brokerageSetType ? $t('message.productList.percent') : $t('message.productList.yuan') }}</span>
                      </template>
                    </el-input>
                    <div class="acea-row row-right row-middle ml14">
                      <el-button size="small" @click="closePop">{{ $t('message.productList.cancel') }}</el-button>
                      <el-button size="small" type="primary" @click="brokerageOneSetUp">{{ $t('message.productList.confirm') }}</el-button>
                    </div>
                  </div>
              ...
              <template slot-scope="scope">
                ...
                >
                  <template slot="suffix">
                    <span>{{ formData.is_sub ? $t('message.productList.yuan') : $t('message.productList.percent') }}</span>
                  </template>
                </el-input>
                ...
                <!-- <div class="flex-x-center red" v-show="formData.is_sub == 1 && scope.row.brokerage == 0">
                  {{ $t('message.productList.commissionCannotBeZero') }}
                </div> -->
                <div
                  class="flex-x-center red"
                  v-show="formData.is_sub == 1 && Number(scope.row.brokerage) > Number(scope.row.price)"
                >
                  {{ $t('message.productList.commissionCannotExceedPrice') }}
                </div>
              </template>
            </el-table-column>
            <el-table-column min-width="120" align="center">
              <template slot="header" slot-scope="scope">
                <span>{{ $t('message.productList.secondLevelRebate') }}</span>
                <el-popover
                  ref="popoverRef_two"
                  placement="top"
                  width="280"
                  ...
                  v-if="formData.is_sub == 1"
                >
                  <div class="pop-title">{{ $t('message.productList.batchSetSecondLevelRebate') }}</div>
                  <div class="mt-14">
                    <el-radio-group v-model="brokerageSetType">
                      <el-radio :label="0">{{ $t('message.productList.specifiedPrice') }}</el-radio>
                      <el-radio :label="1">{{ $t('message.productList.ratio2') }}</el-radio>
                  ...
                  <div class="mt10 mb10 acea-row">
                    ...
                    <div class="acea-row row-right row-middle ml14">
                      <el-button size="small" @click="closePop">{{ $t('message.productList.cancel') }}</el-button>
                      <el-button size="small" type="primary" @click="brokerageTwoSetUp">{{ $t('message.productList.confirm') }}</el-button>
                    </div>
                  </div>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="onCancel">{{ $t('message.productList.cancel') }}</el-button>
        <el-button type="primary" @click="submitForm" :disabled="disabled" class="ml-14">{{ $t('message.productList.confirm') }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      if (!isSuccess) return this.$message.error(this.$t('message.productList.commissionCannotBeZero'));
      if (!step) return this.$message.error(this.$t('message.productList.commissionCannotBeGreaterThanPrice'));
      this.disabled = true;
      let data = {
        ...this.formData,
    ...
    brokerageOneSetUp() {
      if (this.brokerage == 0) return this.$message.error(this.$t('message.productList.priceAndDiscountCannotBeZero'));
      if (this.brokerageSetType == 1 && this.brokerage > 100) return this.$message.error(this.$t('message.productList.discountCannotExceed100'));
    ...
    brokerageTwoSetUp() {
      if (this.brokerage_two == 0) return this.$message.error(this.$t('message.productList.priceAndDiscountCannotBeZero'));
      if (this.brokerageSetType == 1 && this.brokerage_two > 100) return this.$message.error(this.$t('message.productList.discountCannotExceed100'));
      this.attrData.map((item) => {
        if (this.brokerageSetType == 0) {
