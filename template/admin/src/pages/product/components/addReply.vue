<template>
  <el-dialog
    ...
    :title="$t('message.productReply.addSelfReview')"
    width="720px"
    :close-on-click-modal="false"
    @close="onCancel"
  >
    <el-form :model="formData" label-width="100px" label-position="right">
      <el-form-item :label="$t('message.productReply.product') + '：'">
      ...
      <el-form-item v-if="goods.id" :label="$t('message.productReply.productSpec') + '：'">
      ...
      <el-form-item :label="$t('message.productReply.userName') + '：'">
        <el-input
          ...
          :placeholder="$t('message.productReply.userNamePlaceholder')"
          class="w100"
          maxlength="20"
          show-word-limit
        ></el-input>
      </el-form-item>
      <el-form-item :label="$t('message.productReply.commentText') + '：'">
        <el-input
          v-model="formData.comment"
          type="textarea"
          :placeholder="$t('message.productReply.commentTextPlaceholder')"
          class="w100"
          maxlength="200"
          show-word-limit
        ></el-input>
      </el-form-item>
      <el-form-item :label="$t('message.productReply.productScore') + '：'">
        <el-rate v-model="product_score" />
      </el-form-item>
      <el-form-item :label="$t('message.productReply.serviceScore') + '：'">
        <el-rate v-model="service_score" />
      </el-form-item>
      <el-form-item :label="$t('message.productReply.commentImage') + '：'">
        <div class="df-aic">
    ...
    <span slot="footer" class="dialog-footer">
      <el-button v-db-click @click="onCancel">{{ $t('message.productCommon.cancel') }}</el-button>
      <el-button type="primary" v-db-click @click="onOk">{{ $t('message.productCommon.confirm') }}</el-button>
    </span>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    onOk() {
      if (!this.goods.id) {
        return this.$message.error(this.$t('message.productReply.selectProduct'));
      }
      if (!this.attr.pic) {
        return this.$message.error(this.$t('message.productReply.selectProductSpec'));
      }
      if (!this.avatar.att_dir) {
        return this.$message.error(this.$t('message.productReply.selectUserAvatar'));
      }
      if (!this.formData.nickname) {
        return this.$message.error(this.$t('message.productReply.enterUserNickname'));
      }
      if (!this.formData.comment) {
        return this.$message.error(this.$t('message.productReply.enterCommentContent'));
      }
      if (!this.product_score) {
        return this.$message.error(this.$t('message.productReply.productScoreRange'));
      }
      if (!this.service_score) {
        return this.$message.error(this.$t('message.productReply.serviceScoreRange'));
      }
      let data = {
        image: {
