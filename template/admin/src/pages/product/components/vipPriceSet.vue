<template>
  <div>
    <el-dialog :visible.sync="visible" :title="$t('message.productList.customMemberPrice')" width="900"
      ><el-form :model="formData" label-width="120px">
        <el-form-item :label="$t('message.productList.paidMemberExclusive')">
          <el-switch
        ...
        <el-form-item :label="$t('message.productList.paidMemberPrice')">
          <el-switch
          ...
          <el-table size="small" border max-height="460" :data="attrData" style="width: 100%">
            ...
            <el-table-column min-width="140" align="center" v-if="formData.is_vip == 1">
              <template slot="header" slot-scope="scope">
                <span>{{ $t('message.productList.paidMemberPrice2') }}</span>
                <el-popover
                  ref="vipSetPopover"
                  ...
                  trigger="click"
                >
                  <div class="pop-title">{{ $t('message.productList.batchModifyThisColumn') }}</div>
                  <div class="mt-14">
                    <el-radio-group v-model="vipSetType">
                      <el-radio :label="0">{{ $t('message.productList.specifiedPrice') }}</el-radio>
                      <el-radio :label="1">{{ $t('message.productList.discount') }}</el-radio>
                      <el-radio :label="2">{{ $t('message.productList.reduceCash') }}</el-radio>
                    </el-radio-group>
                  </div>
                  <div class="mt10 mb10 acea-row row-middle">
                    <span class="mr5" v-show="vipSetType == 2">{{ $t('message.productList.reduce') }}</span>
                    <el-input type="number" class="popover-input" v-model="vipSetNum">
                      <template slot="suffix">
                        <span v-show="vipSetType == 0">{{ $t('message.productList.yuan') }}</span>
                        <span v-show="vipSetType == 1">{{ $t('message.productList.percent') }}</span>
                      </template>
                    </el-input>
                    <div class="acea-row row-right row-middle ml14">
                      <el-button size="small" @click="closeVipSet">{{ $t('message.productList.cancel') }}</el-button>
                      <el-button size="small" type="primary" class="ml-14" @click="vipSetConfirm">{{ $t('message.productList.confirm') }}</el-button>
                    </div>
                  </div>
              ...
              <template slot-scope="scope">
                ...
                <div class="flex-x-center red" v-show="scope.row.vip_price == 0">{{ $t('message.productList.memberPriceCannotBeZero') }}</div>
                <div class="flex-x-center red" v-show="Number(scope.row.vip_price) > Number(scope.row.price)">
                  {{ $t('message.productList.memberPriceCannotExceedPrice') }}
                </div>
              </template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      if (!isSuccess) return this.$message.error(this.$t('message.productList.memberPriceCannotBeZero'));
      if (!step) return this.$message.error(this.$t('message.productList.memberPriceCannotExceedPrice'));
      this.disabled = true;
      let data = {
        ...this.formData,
    ...
    vipSetConfirm() {
      if (this.vipSetNum == 0) return this.$message.error(this.$t('message.productList.memberPriceCannotBeZero'));
      if (this.vipSetType == 1 && this.vipSetNum > 100) return this.$message.error(this.$t('message.productList.discountCannotExceed100'));
    ...
    levelSetConfirm(index) {
      if (this.levelSetNum == 0) return this.$message.error(this.$t('message.productList.levelMemberPriceCannotBeZero'));
      if (this.levelSetType == 1 && this.levelSetNum > 100) return this.$message.error(this.$t('message.productList.discountCannotExceed100'));
      this.attrData.map((item) => {
