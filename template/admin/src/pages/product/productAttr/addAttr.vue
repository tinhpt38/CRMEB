<template>
  <el-dialog :visible.sync="modal" @closed="onCancel" :title="$t('message.productSpec.addProductSpec')" width="1000px" v-loading="spinShow">
    <el-form
      ref="formDynamic"
      :model="formDynamic"
      :rules="rules"
    ...
    >
      <el-row :gutter="24">
        <el-col :span="24">
          <el-col :span="8">
            <el-form-item :label="$t('message.productSpec.templateName') + '：'" prop="rule_name">
              <el-input :placeholder="$t('message.productSpec.enterSpecName')" :maxlength="20" v-model.trim="formDynamic.rule_name" />
            </el-form-item>
          </el-col>
        </el-col>
        <el-col :span="23" class="noForm" :key="index">
          <el-form-item label="">
            <div class="specifications">
                ...
                <div class="specifications-item active" v-for="(item, index) in formDynamic.spec" :key="index">
                  ...
                  <div class="specifications-item-box">
                    ...
                    <div class="specifications-item-name mb18">
                      <el-input
                        v-model="item.value"
                        :placeholder="$t('message.productSpec.enterSpecName')"
                      ...
                      <draggable class="item" :list="item.detail" handle=".drag">
                        <div v-for="(j, indexn) in item.detail" :key="indexn" class="mr10 spec drag">
                          ...
                          <el-input
                            ...
                            :placeholder="$t('message.productSpec.specName')"
                            maxlength="30"
                          >
                        ...
                        >
                          <el-input
                            :ref="'inputRef_' + index"
                            :placeholder="$t('message.productSpec.enterSpecAttr')"
                          ...
                          <div class="addfont" slot="reference" type="text" v-db-click>{{ $t('message.productSpec.addSpecValue') }}</div>
                        </el-popover>
                      </draggable>
                    </div>
                  </div>
              ...
              <el-button v-if="formDynamic.spec.length < 4" v-db-click @click="handleAddRole()">{{ $t('message.productSpec.addNewSpec') }}</el-button>
            </div>
          </el-form-item>
        </el-col>
      </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      index: 1,
      rules: {
        rule_name: [{ required: true, message: this.$t('message.productSpec.enterSpecName'), trigger: 'blur' }],
      },
      formDynamic: {
        rule_name: '',
        spec: [],
  ...
  methods: {
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          if (this.formDynamic.spec.length === 0) {
            return this.$message.warning(this.$t('message.productSpec.selectSpecFirst'));
          }
    ...
    createAttrName() {
      if (this.attrsName && this.attrsVal) {
        ...
        this.$message.warning(this.$t('message.productSpec.addSpecNameOrValue'));
      }
    },
    // 添加属性
    createAttr(num, idx) {
      if (num) {
        ...
        this.$message.warning(this.$t('message.productSpec.addAttribute'));
      }
    },
  },
};
