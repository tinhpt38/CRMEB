  ...
  <div>
    <div class="main">
      <el-card :bordered="false" shadow="never" class="ivu-mt mr20 card-tree">
        <div class="tree">
          <div class="main-btn">
            <el-button class="mb10" type="primary" v-db-click @click="clickMenu(4)" long>{{ $t('pages.setting.systemOutInterface.addCategory') }}</el-button>
          </div>
          ...
          <vue-tree-list
            ...
            @delete-node="onDel"
            :model="treeData"
            :default-tree-node-name="$t('pages.setting.systemOutInterface.defaultFolder')"
            :default-leaf-node-name="$t('pages.setting.systemOutInterface.defaultApi')"
              ...
              >
                ...
                >
                  ...
                  <template slot="dropdown">
                    <el-dropdown-menu>
                      <el-dropdown-item command="1" v-if="!slotProps.model.method">{{ $t('pages.setting.systemOutInterface.addApi') }}</el-dropdown-item>
                      <el-dropdown-item command="2" v-if="!slotProps.model.method">{{ $t('pages.setting.systemOutInterface.editCategoryName') }}</el-dropdown-item>
                      <el-dropdown-item command="3">{{ $t('pages.setting.systemOutInterface.del') }}</el-dropdown-item>
                    </el-dropdown-menu>
                  </template>
                </el-dropdown>
              </div>
      ...
      <el-card :bordered="false" shadow="never" class="ivu-mt right-card">
        <div class="data">
          <div class="eidt-sub">
            ...
            <div>
              ...
              <el-button
                ...
                v-db-click
                @click="isEdit = !isEdit"
                >{{ isEdit ? $t('pages.setting.systemOutInterface.back') : $t('pages.setting.systemOutInterface.edit') }}</el-button
              >
              <el-button
                v-if="isEdit"
                type="primary"
                ...
                >{{ $t('pages.setting.systemOutInterface.save') }}</el-button
              >
            </div>
          </div>
          <el-form
          ...
          >
            <el-row :gutter="24">
              <el-col :span="24">
                <div class="title">{{ $t('pages.setting.systemOutInterface.apiInfo') }}</div>
                <el-form-item :label="$t('pages.setting.systemOutInterface.apiName')" prop="name">
                  <el-input
                    v-if="isEdit"
                    class="perW20"
                    type="text"
                    ...
                    :placeholder="$t('pages.setting.systemOutInterface.placeholderInput')"
                  />
                  <span v-else>{{ formValidate.name || '' }}</span>
                </el-form-item>
                <el-form-item :label="$t('pages.setting.systemOutInterface.requestType')" prop="name">
                ...
                <el-form-item :label="$t('pages.setting.systemOutInterface.funcDesc')" prop="name">
                  <el-input
                    v-if="isEdit"
                    class="perW20"
                    type="textarea"
            ...
            <el-row :gutter="24">
              <el-col :span="24">
                <div class="title">{{ $t('pages.setting.systemOutInterface.callMethod') }}</div>
                <el-form-item :label="$t('pages.setting.systemOutInterface.callContent')" prop="url">
                  <el-input
                    v-if="isEdit"
                    class="perW20"
                    type="text"
                ...
                <el-form-item :label="$t('pages.setting.systemOutInterface.requestParams')">
                  <vxe-table
                    resizable
                    show-overflow
                    keep-source
                  ...
                  >
                    ...
                    <vxe-column field="attribute" width="300" :title="$t('pages.setting.systemOutInterface.attribute')" tree-node :edit-render="{}">
                      <template #default="{ row }">
                        <vxe-input v-if="isEdit" v-model="row.attribute" type="text"></vxe-input>
                        <span v-else>{{ row.attribute || '' }}</span>
                      </template>
                    </vxe-column>
                    <vxe-column field="type" :title="$t('pages.setting.systemOutInterface.type')" width="200" :edit-render="{}">
                      <template #default="{ row }">
                    ...
                    <vxe-column field="must" :title="$t('pages.setting.systemOutInterface.required')" width="100" :edit-render="{}">
                      <template #default="{ row }">
                        <vxe-checkbox
                          v-if="isEdit"
                          v-model="row.must"
                          ...
                          :checked-value="'1'"
                        ></vxe-checkbox>
                        <span v-else>{{ row.must == '1' ? $t('pages.setting.systemOutInterface.yes') : $t('pages.setting.systemOutInterface.no') }}</span>
                      </template>
                    </vxe-column>
                    <vxe-column field="trip" :title="$t('pages.setting.systemOutInterface.desc')" :edit-render="{}">
                      <template #default="{ row }">
                        <vxe-input v-if="isEdit" v-model="row.trip" type="text"></vxe-input>
                        <span v-else>{{ row.trip || '' }}</span>
                      </template>
                    </vxe-column>
                    <vxe-column :title="$t('pages.setting.systemOutInterface.action')" width="200" v-if="isEdit">
                      <template #default="{ row }">
                        <vxe-button
                          type="text"
                          v-if="row.type === 'array'"
                          ...
                          >{{ $t('pages.setting.systemOutInterface.insert') }}</vxe-button
                        >
                        <vxe-button type="text" status="primary" v-db-click @click="removeRow(row, 'xTable')"
                          >{{ $t('pages.setting.systemOutInterface.del') }}</vxe-button
                        >
                      </template>
                    </vxe-column>
                  </vxe-table>
                  ...
                  <el-button class="mt10" v-if="isEdit" type="primary" v-db-click @click="insertEvent('xTable')"
                    >{{ $t('pages.setting.systemOutInterface.addParam') }}</el-button
                  >
                </el-form-item>
                <el-form-item :label="$t('pages.setting.systemOutInterface.returnParams')">
                  <vxe-table
                    resizable
                    show-overflow
                    keep-source
                  ...
                  >
                    <!-- <vxe-column type="checkbox" width="60"></vxe-column> -->
                    <vxe-column field="attribute" :title="$t('pages.setting.systemOutInterface.attribute')" width="300" tree-node :edit-render="{}">
                      <template #default="{ row }">
                        <vxe-input v-if="isEdit" v-model="row.attribute" type="text"></vxe-input>
                        <span v-else>{{ row.attribute || '' }}</span>
                      </template>
            ...
            <el-row :gutter="24">
              <el-col :span="24">
                <div class="title">{{ $t('pages.setting.systemOutInterface.callExample') }}</div>
                <el-form-item :label="$t('pages.setting.systemOutInterface.requestExample')" prop="request_example">
                  <el-input
                    v-if="isEdit"
                    class="perW20"
                    type="textarea"
                ...
                <el-form-item :label="$t('pages.setting.systemOutInterface.returnExample')" prop="return_example">
                  <el-input
                    v-if="isEdit"
                    class="perW20"
                    type="textarea"
                ...
                <el-form-item :label="$t('pages.setting.systemOutInterface.errorCode')">
                  <vxe-table
                    resizable
                    show-overflow
                    keep-source
                  ...
                  >
                    <!-- <vxe-column type="checkbox" width="60"></vxe-column> -->
                    <vxe-column field="code" :title="$t('pages.setting.systemOutInterface.errorCode')" tree-node :edit-render="{}">
                      <template #default="{ row }">
                        <vxe-input v-if="isEdit" v-model="row.code" type="text"></vxe-input>
                        <span v-else>{{ row.code || '' }}</span>
                      </template>
                    </vxe-column>
                    <vxe-column field="value" :title="$t('pages.setting.systemOutInterface.errorCodeValue')" :edit-render="{}">
                      <template #default="{ row }">
                        <vxe-input v-if="isEdit" v-model="row.value" type="text"></vxe-input>
                        <span v-else>{{ row.value || '' }}</span>
                      </template>
                    </vxe-column>
                    <vxe-column field="solution" :title="$t('pages.setting.systemOutInterface.solution')" :edit-render="{}">
                      <template #default="{ row }">
                        <vxe-input v-if="isEdit" v-model="row.solution" type="text"></vxe-input>
                        <span v-else>{{ row.solution || '' }}</span>
                      </template>
                    </vxe-column>
                    <vxe-column :title="$t('pages.setting.systemOutInterface.action')" v-if="isEdit">
                      <template #default="{ row }">
    ...
    <el-dialog :visible.sync="nameModal" width="470px" :title="$t('pages.setting.systemOutInterface.groupName')" @on-ok="asyncOK">
      <label>{{ $t('pages.setting.systemOutInterface.groupNameLabel') }}</label>
      <el-input v-model="value" :placeholder="$t('pages.setting.systemOutInterface.placeholderGroupName')" style="width: 85%" />
      <span slot="footer" class="dialog-footer">
        <el-button v-db-click @click="nameModal = false">{{ $t('pages.setting.systemOutInterface.cancel') }}</el-button>
        <el-button type="primary" v-db-click @click="asyncOK">{{ $t('pages.setting.systemOutInterface.confirm') }}</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  created() {
    this.ruleValidate = {
      title: [{ message: this.$t('pages.setting.systemOutInterface.descMax'), trigger: 'blur', max: 200 }],
    };
  ...
  methods: {
    ...
    async handleSubmit() {
      if (!this.formValidate.name) {
        return this.$message.warning(this.$t('pages.setting.systemOutInterface.apiNameRequired'));
      } else if (!this.formValidate.method) {
        return this.$message.warning(this.$t('pages.setting.systemOutInterface.requestTypeRequired'));
      } else if (!this.formValidate.url) {
        return this.$message.warning(this.$t('pages.setting.systemOutInterface.callMethodRequired'));
      }
    ...
    onDel(node) {
      this.$msgbox({
        title: this.$t('pages.setting.systemOutInterface.tip'),
        message: this.$t('pages.setting.systemOutInterface.delConfirm'),
        showCancelButton: true,
        cancelButtonText: this.$t('pages.setting.systemOutInterface.cancel'),
        confirmButtonText: this.$t('pages.setting.systemOutInterface.confirm'),
