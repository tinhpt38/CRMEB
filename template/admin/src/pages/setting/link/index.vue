  ...
  <el-card :bordered="false" shadow="never" class="ivu-mt">
    <div class="Modal">
      <div class="colLeft">
                    ...
                    <span>
                      <el-dropdown @command="(command) => clickMenu(data, command)">
                        ...
                        <template slot="dropdown">
                          <el-dropdown-menu>
                            <el-dropdown-item v-if="data.pid == 1" command="1">{{ $t('message.setting.addCategory') }}</el-dropdown-item>
                            <el-dropdown-item v-if="data.id" command="2">{{ $t('message.setting.editCategory') }}</el-dropdown-item>
                            <el-dropdown-item v-if="data.id" command="3">{{ $t('message.setting.delete') }}</el-dropdown-item>
                          </el-dropdown-menu>
                        </template>
                      </el-dropdown>
                    </span>
        ...
        <div class="conter">
          <el-button type="primary" @click="addLink">{{ $t('message.setting.addLink') }}</el-button>
          <el-table
            :data="linkList"
            ref="couponTable"
            class="mt20"
          ...
          >
            <el-table-column :label="$t('message.common.id')" width="70">
              <template slot-scope="scope">
                <span>{{ scope.row.id }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('message.setting.name')" width="150">
              <template slot-scope="scope">
                <span>{{ scope.row.name }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('message.setting.h5Link')" minWidth="170">
              <template slot-scope="scope">
                <el-tooltip class="item pointer" :content="$t('message.setting.clickToCopy')">
                  <span v-db-click @click="onCopy(scope.row.h5_url)">{{ scope.row.h5_url }}</span>
                </el-tooltip>
              </template>
            </el-table-column>
            <el-table-column :label="$t('message.setting.miniProgramLink')" minWidth="140">
              <template slot-scope="scope">
                <el-tooltip class="item pointer" :content="$t('message.setting.clickToCopy')">
            ...
            <el-table-column :label="$t('message.setting.addTime')" minWidth="90">
              <template slot-scope="scope">
            ...
            <el-table-column :label="$t('message.common.operation')" fixed="right" width="90">
              <template slot-scope="scope">
                ...
                <a v-db-click @click="del(scope.row, $t('message.setting.deleteLink'), scope.$index)">{{ $t('message.setting.delete') }}</a>
              </template>
            </el-table-column>
          </el-table>
    ...
    >
      <el-form :model="linkForm" ref="linkForm" label-width="80px">
        <el-form-item :label="$t('message.setting.name') + ':'" prop="name">
          <el-input v-model="linkForm.name" :placeholder="$t('message.setting.pleaseInputName')"></el-input>
        </el-form-item>
        <el-form-item :label="$t('message.setting.jumpLink') + ':'" prop="url">
          <el-input v-model="linkForm.url" :placeholder="$t('message.setting.pleaseInputJumpLink')"></el-input>
        </el-form-item>
        <el-form-item :label="$t('message.setting.sort') + ':'" prop="url">
          <el-input v-model="linkForm.sort" :placeholder="$t('message.setting.pleaseInputSort')"></el-input>
        </el-form-item>
        <el-form-item :label="$t('message.setting.isEnable') + ':'" prop="url">
          <el-switch v-model="linkForm.status" :active-value="1" :inactive-value="0"></el-switch>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false">{{ $t('message.setting.cancel') }}</el-button>
        <el-button type="primary" @click="addLinkSubmit">{{ $t('message.setting.confirm') }}</el-button>
      </div>
    </el-dialog>
  </el-card>
</template>
...
export default {
  ...
  methods: {
    ...
    remove(data, tit) {
      this.tits = tit;
      let delfromData = {
        title: this.$t('message.setting.delete') + ' [ ' + data.title + ' ] ' + this.$t('message.setting.category'),
