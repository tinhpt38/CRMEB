<template>
  <div>
    <el-card :bordered="false" shadow="never" class="ivu-mb-16" :body-style="{ padding: 0 }">
      <div class="padding-add">
        <el-form
          ...
          inline
        >
          <el-form-item :label="$t('message.systemCommon.status') + '：'" label-for="status1">
            <el-select v-model="status" :placeholder="$t('message.systemCommon.pleaseSelect')" @change="userSearchs" clearable class="form_content_width">
              <el-option value="all" :label="$t('message.systemCommon.all')"></el-option>
              <el-option value="1" :label="$t('message.systemCommon.enabled')"></el-option>
              <el-option value="0" :label="$t('message.systemCommon.disabled')"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item :label="$t('message.setting.searchTitle') + '：'" label-for="status2">
            <el-input
              clearable
              :placeholder="$t('message.setting.pleaseEnterNameOrAccount')"
    ...
    <el-card :bordered="false" shadow="never" class="ivu-mt">
      ...
      >
        <el-table-column :label="$t('message.setting.name')" width="120">
        ...
        <el-table-column :label="$t('message.setting.role')" min-width="130">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('message.systemCommon.status')" min-width="70">
          <template slot-scope="scope">
            <el-switch
              class="defineSwitch"
              :active-value="1"
        ...
        <el-table-column :label="$t('message.systemCommon.operation')" fixed="right" width="100">
          <template slot-scope="scope">
            <a v-db-click @click="edit(scope.row)">{{ $t('message.systemCommon.edit') }}</a>
            <el-divider direction="vertical"></el-divider>
            <a v-db-click @click="del(scope.row, $t('message.setting.deleteAdminConfirm'), scope.$index)">{{ $t('message.systemCommon.delete') }}</a>
          </template>
        </el-table-column>
      </el-table>
