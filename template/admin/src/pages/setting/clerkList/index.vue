<template>
  <div>
    <el-card :bordered="false" shadow="never" class="ivu-mb-16" :body-style="{ padding: 0 }">
      <div class="padding-add">
        <el-form
          ...
          inline
        >
          <el-form-item :label="$t('message.setting.clerkList.pickupPointName') + '：'">
    ...
    <el-card :bordered="false" shadow="never" class="ivu-mt">
      <el-button v-auth="['merchant-store_staff-create']" type="primary" v-db-click @click="add">{{ $t('message.setting.clerkList.addClerk') }}</el-button>
      <el-table
        :data="storeLists"
        ref="table"
        class="mt14"
      ...
      >
        ...
        <el-table-column :label="$t('message.setting.clerkList.avatar')" min-width="90">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('message.setting.clerkList.userName')" min-width="130">
          <template slot-scope="scope">
            <span>{{ scope.row.nickname }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('message.setting.clerkList.clerkName')" min-width="130">
          <template slot-scope="scope">
            <span>{{ scope.row.staff_name }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('message.setting.clerkList.affiliatedPickupPoint')" min-width="130">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('message.systemCommon.status')" min-width="150">
          <template slot-scope="scope">
            <el-switch
              class="defineSwitch"
              :active-value="1"
        ...
        <el-table-column :label="$t('message.systemCommon.operation')" fixed="right" width="170">
          <template slot-scope="scope">
            ...
            <a v-db-click @click="del(scope.row, $t('message.setting.clerkList.deleteClerk'), scope.$index)">{{ $t('message.systemCommon.delete') }}</a>
          </template>
        </el-table-column>
      </el-table>
