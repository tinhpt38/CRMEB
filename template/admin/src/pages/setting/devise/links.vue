<template>
  <div class="right-box">
    <div class="link-item" v-for="(item, index) in list" :key="index">
      ...
      <div class="txt"><span>{{ $t('router.setting.devise.address') }}</span>{{ item.url }}</div>
      <div class="txt" v-if="item.parameter">
        <p><span>{{ $t('router.setting.devise.params') }}</span></p>
        <span v-for="(val, key, index) in item.parameter" :key="index"
          >{{ key + '=' + val }}<i style="font-style: normal">&</i></span
        >
      </div>
      <div class="tips">
        {{ $t('router.setting.devise.example') }}：{{ item.example }}
        <!--<el-button size="small" style="margin-left: 10px" v-clipboard:copy="item.example"-->
        <!--v-clipboard:success="onCopy"-->
        <!--v-clipboard:error="onError">复制</el-button>-->
        <span class="copy copy-data" v-db-click @click="onCopy(item.example)">{{ $t('router.setting.devise.copy') }}</span>
      </div>
    </div>
  </div>
</template>
