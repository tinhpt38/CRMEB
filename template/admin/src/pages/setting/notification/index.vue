  ...
  <div class="message">
    <el-card :bordered="false" shadow="never" class="ivu-mt" :body-style="{ padding: '0 20px 20px' }">
      ...
      <el-row class="mb14" v-if="currentTab == 1">
        <el-col>
          <el-button v-auth="['app-wechat-template-sync']" type="primary" v-db-click @click="routineTemplate"
            >{{ $t('message.notification.syncMiniProgram') }}</el-button
          >
          <el-button v-auth="['app-wechat-template-sync']" type="primary" v-db-click @click="wechatTemplate"
            >{{ $t('message.notification.syncWechatTemplate') }}</el-button
          >
        </el-col>
      </el-row>
      <el-row class="mb14" v-if="currentTab == 3">
        <el-col>
          <el-button type="primary" v-db-click @click="notificationForm(0)">{{ $t('message.notification.addNotification') }}</el-button>
        </el-col>
      </el-row>
      ...
      >
        ...
        <el-table-column :label="$t('message.notification.notificationType')" min-width="130">
          <template slot-scope="scope">
            <span>{{ scope.row.name }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('message.notification.notificationScene')" min-width="130">
          <template slot-scope="scope">
            <span>{{ scope.row.title }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('message.notification.internalMessage')" min-width="130">
          <template slot-scope="scope">
            <el-switch
        ...
        <el-table-column :label="$t('message.notification.wechatTemplate')" min-width="130">
          <template slot-scope="scope">
            <el-switch
        ...
        <el-table-column :label="$t('message.notification.sendSms')" min-width="130">
          <template slot-scope="scope">
            <el-switch
        ...
        <el-table-column :label="$t('message.notification.enterpriseWechat')" min-width="130" v-if="currentTab != 1">
          <template slot-scope="scope">
            <el-switch
        ...
        <el-table-column :label="$t('message.notification.miniProgramSubscribe')" min-width="130" v-if="currentTab == 1 || currentTab == 3">
          <template slot-scope="scope">
            <el-switch
        ...
        <el-table-column :label="$t('message.systemCommon.operation')" fixed="right" :width="currentTab == 3 ? 130 : 70">
          <template slot-scope="scope">
            <a class="setting btn" v-db-click @click="setting(scope.row)">{{ $t('message.notification.settings') }}</a>
            <template v-if="currentTab == 3">
              <el-divider direction="vertical"></el-divider>
              <a class="setting btn" v-db-click @click="notificationForm(scope.row.id)">{{ $t('message.systemCommon.edit') }}</a>
              <el-divider direction="vertical"></el-divider>
              <a class="setting btn" v-db-click @click="del(scope.row, $t('message.notification.deleteNotification'), scope.$index)">{{ $t('message.systemCommon.delete') }}</a>
            </template>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  data() {
    return {
      ...
      headerList: [
        { label: this.$t('message.notification.memberNotification'), value: '1' },
        { label: this.$t('message.notification.platformNotification'), value: '2' },
        { label: this.$t('message.notification.customNotification'), value: '3' },
      ],
      levelLists: [],
      currentTab: '1',
      loading: false,
