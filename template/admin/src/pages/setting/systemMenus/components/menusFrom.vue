  ...
  <div>
    <el-dialog :visible.sync="modals" width="540px" :title="titleFrom" :close-on-click-modal="false" @closed="visible">
      <el-form ref="formValidate" :model="formValidate" label-width="80px" @submit.native.prevent>
        <el-row :gutter="24">
          <el-col v-bind="grid">
            <el-form-item :label="(!authType ? $t('message.systemMenus.apiName') : $t('message.systemMenus.buttonName')) + '：'" prop="menu_name">
              <div class="add">
                <el-input
                  v-model="formValidate.menu_name"
                  :placeholder="!authType ? $t('message.systemMenus.enterApiName') : $t('message.systemMenus.enterButtonName')"
                >
                </el-input>
              </div>
            </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button v-db-click @click="handleReset">{{ $t('message.systemCommon.cancel') }}</el-button>
        <el-button type="primary" v-db-click @click="handleSubmit('formValidate')" :disabled="valids">{{ $t('message.systemCommon.submit') }}</el-button>
      </span>
    </el-dialog>
    <el-dialog :visible.sync="modal12" width="720px" :title="$t('message.systemMenus.iconSelect')">
      <el-input
        v-model="iconVal"
        :placeholder="$t('message.systemMenus.searchIconPlaceholder')"
        clearable
        style="width: 300px"
    ...
    <el-dialog :visible.sync="ruleModal" width="1100px" :title="$t('message.systemMenus.permissionList')" @closed="modalchange">
      ...
      <div class="rule">
        ...
        >
          ...
          <div>{{ $t('message.systemMenus.apiPath') }}：{{ item.rule }}</div>
        </div>
      </div>
    </el-dialog>
  </div>
    ...
    handleSubmit(name) {
      ...
      if (this.authType) {
        if (!this.formValidate.menu_name) {
          return this.$message.warning(this.$t('message.systemMenus.pleaseEnterButtonName'));
        }
        if (!this.formValidate.menu_path) {
          return this.$message.warning(this.$t('message.systemMenus.pleaseEnterRoutePath'));
        }
      } else {
        if (!this.formValidate.menu_name) {
          return this.$message.warning(this.$t('message.systemMenus.pleaseEnterApiName'));
        }
        if (!this.formValidate.methods) {
          return this.$message.warning(this.$t('message.systemMenus.pleaseSelectRequestMethod'));
        }
        if (!this.formValidate.api_url) {
          return this.$message.warning(this.$t('message.systemMenus.pleaseSelectApiPath'));
        }
      }
      this.valids = true;
      addMenusApi(data)
