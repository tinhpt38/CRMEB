<template>
  <div>
    <el-card :bordered="false" shadow="never" class="ivu-mt">
      <el-row>
        <el-col v-bind="grid">
          <el-button type="primary" v-db-click @click="add">{{ $t('message.setting.lang.addLanguage') }}</el-button>
        </el-col>
      </el-row>
      <el-table
        :data="list"
        ...
        :empty-text="$t('message.systemCommon.noData')"
      >
        ...
        <el-table-column :label="$t('message.setting.lang.languageName')" min-width="200">
          <template slot-scope="scope">
            <div class="acea-scope.row scope.row-middle">
              <span>{{ scope.row.language_name }}</span>
              <el-tag class="ml10" color="default" v-if="scope.row.is_default">{{ $t('message.setting.lang.isDefault') }}</el-tag>
            </div>
          </template>
        </el-table-column>
        <el-table-column :label="$t('message.setting.lang.browserLangCode')" min-width="130">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('message.systemCommon.operation')" fixed="right" width="170">
          <template slot-scope="scope">
            <a v-db-click @click="edit(scope.row, $t('message.setting.lang.editLanguage'), index)">{{ $t('message.setting.lang.edit') }}</a>
