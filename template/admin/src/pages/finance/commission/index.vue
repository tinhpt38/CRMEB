    ...
    <el-card :bordered="false" shadow="never" :body-style="{ padding: 0 }">
      <div class="padding-add">
        <el-form ref="formValidate" :label-width="labelWidth" label-position="right" inline @submit.native.prevent>
          <el-form-item :label="$t('message.pages.finance.commission.nicknameId')">
            <el-input :placeholder="$t('message.pages.finance.commission.placeholderInput')" v-model="formValidate.nickname" clearable class="form_content_width" />
          </el-form-item>
          <el-form-item :label="$t('message.pages.finance.commission.commissionRange')" class="tab_data">
            <el-input-number :controls="false" :min="0" class="mr10" v-model="formValidate.price_min" />
            <span class="mr10">{{ $t('message.pages.finance.commission.rangeTo') }}</span>
            <el-input-number :controls="false" :min="0" v-model="formValidate.price_max" />
          </el-form-item>
          <el-form-item>
            <el-button type="primary" v-db-click @click="userSearchs">{{ $t('message.pages.finance.commission.query') }}</el-button>
          </el-form-item>
        </el-form>
      </div>
    </el-card>
    <el-card :bordered="false" shadow="never" class="mt16">
      <el-button v-auth="['export-userCommission']" class="export" v-db-click @click="exports">{{ $t('message.pages.finance.commission.export') }}</el-button>
      <el-table
        ref="table"
        :data="tabList"
        v-loading="loading"
        :empty-text="$t('message.pages.finance.commission.noData')"
        @on-sort-change="sortChanged"
        class="mt14"
      >
        <el-table-column :label="$t('message.pages.finance.commission.userInfo')" min-width="100">
          <template slot-scope="scope">
            <span>{{ scope.row.nickname }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('message.pages.finance.commission.totalCommission')" min-width="100">
          <template slot-scope="scope">
            <span>{{ scope.row.sum_number }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('message.pages.finance.commission.accountCommission')" min-width="100">
          <template slot-scope="scope">
            <span>{{ scope.row.brokerage_price }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('message.pages.finance.commission.withdrawCommission')" min-width="100">
          <template slot-scope="scope">
