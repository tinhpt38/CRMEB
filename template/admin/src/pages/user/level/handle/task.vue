    ...
    <el-row>
      ...
      <el-col :span="24" class="userAlert">
        <el-alert show-icon closable>
          <template slot="title">
            <div>{{ $t('message.pages.user.level.addLevelTaskDesc') }}</div>
          </template>
        </el-alert>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    getFrom() {
      ...
      const titleKey = this.ids ? 'message.pages.user.level.editLevelTask' : 'message.pages.user.level.addLevelTask';
      this.$modalForm(createTaskApi(data), { titleKey }).then(() => this.getList());
    },
    // 编辑
    edit(row) {
      this.ids = row.id;
