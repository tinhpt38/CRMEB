    ...
    <el-card :bordered="false" shadow="never">
      ...
      >
        <el-row :gutter="24">
          <el-col :span="24">
            <el-col v-bind="grid">
              <el-form-item :label="$t('message.setting.elecInvoice.status') + '：'" prop="name" label-for="name">
                ...
                <div class="tips-info">{{ $t('message.setting.elecInvoice.enableTip') }}</div>
              </el-form-item>
            </el-col>
          </el-col>
          <template v-if="formItem.elec_invoice === 1">
            <el-col :span="24">
              <el-col v-bind="grid">
                <el-form-item :label="$t('message.setting.elecInvoice.autoInvoice') + '：'" prop="name" label-for="name">
                  <div>
                    <el-radio-group v-model="formItem.auto_invoice">
                      <el-radio :label="1">{{ $t('message.systemCommon.enabled') }}</el-radio>
                      <el-radio :label="0">{{ $t('message.systemCommon.disabled') }}</el-radio>
                    </el-radio-group>
                    <div class="tips-info">{{ $t('message.setting.elecInvoice.autoInvoiceTip') }}</div>
                  </div>
                </el-form-item>
              </el-col>
            </el-col>
            <template v-if="formItem.auto_invoice === 1 && formItem.elec_invoice === 1">
              <el-col :span="24">
                <el-col v-bind="grid">
                  <el-form-item :label="$t('message.setting.elecInvoice.category') + '：'">
                    <el-select
                      class="input-width"
                      ...
                      :loading="loading"
                      :placeholder="$t('message.setting.elecInvoice.enterAndSelectCategory')"
                      @change="selectChange"
                    >
                      <el-option v-for="item in options" :key="item.id" :label="item.name" :value="item.id">
                      </el-option>
                    </el-select>
                    <div class="tips-info">{{ $t('message.setting.elecInvoice.categoryTip') }}</div>
                  </el-form-item>
                </el-col>
              </el-col>
              <el-col :span="24">
                <el-col v-bind="grid">
                  <el-form-item :label="$t('message.setting.elecInvoice.taxRate') + '：'">
                    <el-input
                      type="number"
                      class="input-width"
                      v-model="formItem.elec_invoice_tax_rate"
                      :placeholder="$t('message.setting.elecInvoice.enterTaxRate')"
                    />
                    <div class="tips-info">
                      {{ $t('message.setting.elecInvoice.taxRateTip') }}
                    </div>
                  </el-form-item>
                </el-col>
              </el-col>
          ...
          <el-col :span="24">
            <el-col v-bind="grid">
              <el-form-item>
                <el-button type="primary" long v-db-click @click="handleSubmit('formItem')">{{ $t('message.systemCommon.save') }}</el-button>
              </el-form-item>
            </el-col>
          </el-col>
        </el-row>
