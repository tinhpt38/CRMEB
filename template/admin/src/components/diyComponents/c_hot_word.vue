<template>
  <div class="line-box">
    <div class="title">
      <p>{{ $t('message.diyComponents.hotSearch') }}</p>
    ...
    <div class="input-box">
      <draggable class="dragArea list-group" :list="datas[name].list" group="peoples" handle=".icon">
        <div class="input-item" v-for="(item, index) in datas[name].list" :key="index">
          ...
          <el-input v-model="item.val" :placeholder="$t('message.diyComponents.hotWordPlaceholder')" :maxlength="item.maxlength || 10" />
      ...
      <div class="add-btn" v-db-click @click="addHotTxt">
        <el-button
          ...
          >{{ $t('message.diyComponents.addHotWord') }}</el-button
        >
      </div>
    </div>
  </div>
  ...
  methods: {
    addHotTxt() {
      ...
      if (this.datas[this.name].list.length < 20) {
        ...
        this.$message.warning(this.$t('message.diyComponents.maxHotWord'));
      }
    },
    close(index) {
