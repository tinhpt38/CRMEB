<template>
  <div class="custom-design">
    ...
    <div class="header">
      <div class="left">
        <span class="close-btn" @click="$emit('close')"><i class="icon el-icon-arrow-left"></i>{{ $t('message.customDesign.back') }}</span>
        <span class="title">{{ $t('message.customDesign.title') }}</span>
      </div>
      <div class="center"></div>
      <div class="right">
        <el-button size="small" @click="save(false)">{{ $t('message.customDesign.save') }}</el-button>
        <el-button type="primary" size="small" @click="save(true)">{{ $t('message.customDesign.saveAndClose') }}</el-button>
      </div>
    </div>
    ...
    <div class="body">
      ...
      <div class="left-panel">
        ...
        <div class="height-setting">
          <div class="label">{{ $t('message.customDesign.contentHeight') }}</div>
          <div class="control">
            <el-slider
        ...
        <div class="layer-list">
          <div class="title">{{ $t('message.customDesign.selectedComponents') }} ({{ componentData.length }})</div>
      ...
      <div class="center-panel" @contextmenu="handleContextMenu" @mousedown="handleBackgroundClick">
        ...
        <div class="history-controls">
          <div class="operation-bar">
            ...
            <div class="operation-item text" @click="historyVisible = !historyVisible">{{ $t('message.customDesign.history') }}</div>
          </div>
        </div>
        <div class="history-list" v-show="historyVisible">
          <div class="history-header">
            <span>{{ $t('message.customDesign.history') }}</span>
          ...
          <div class="history-content">
            ...
            >
              <span class="type">{{ item.type || $t('message.customDesign.action') }}</span>
        ...
        <div class="component-actions" v-if="curComponent" :style="actionBarStyle">
          <div class="action-item" :title="$t('message.customDesign.delete')" @click="deleteComponent(curComponentIndex)">
            <span class="iconfont iconshanchu3"></span>
          </div>
          <div class="action-item" :title="$t('message.customDesign.copy')" @click="copyComponent(curComponent)">
            <i class="el-icon-document-copy"></i>
          </div>
          <div
            class="action-item"
            :class="{ disabled: isTop }"
            :title="$t('message.customDesign.moveUp')"
          ...
          <div
            ...
            :title="$t('message.customDesign.moveDown')"
          ...
          <div
            class="action-item"
            :class="{ disabled: isTop }"
            :title="$t('message.customDesign.moveTop')"
          ...
          <div
            class="action-item"
            :class="{ disabled: isBottom }"
            :title="$t('message.customDesign.moveBottom')"
      ...
      <div class="right-panel">
        ...
        <div v-else class="empty-tip">
          ...
          <p>{{ $t('message.customDesign.emptySelected') }}</p>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    reversedComponentData: {
      ...
      set(val) {
        ...
        this.recordSnapshot(this.$t('message.customDesign.moveTop'));
      },
    },
  },
  created() {
  ...
  methods: {
    ...
    deleteComponent(index) {
      ...
      this.curComponent = null;
      this.curComponentIndex = -1;
      this.recordSnapshot(this.$t('message.customDesign.delete'));
    },
    toggleHide(item) {
      this.$set(item, 'isHidden', !item.isHidden);
    },
    editName(item) {
      this.$prompt(this.$t('message.customDesign.inputComponentName'), this.$t('message.customDesign.editName'), {
        confirmButtonText: this.$t('message.customDesign.confirm'),
        cancelButtonText: this.$t('message.customDesign.cancel'),
    ...
    recordSnapshot(type = this.$t('message.customDesign.action')) {
      // 简单的撤销重做实现
      this.snapshotIndex++;
    ...
    copyComponent(item) {
      ...
      this.recordSnapshot(this.$t('message.customDesign.copy'));
    },
    moveComponent({ index, type }) {
      ...
      if (type === 'up') {
        if (index < this.componentData.length - 1) {
          ...
          this.recordSnapshot(this.$t('message.customDesign.moveUp'));
        } else {
          this.$message.warning(this.$t('message.customDesign.alreadyTop'));
        }
      } else if (type === 'down') {
        if (index > 0) {
          this.componentData.splice(index, 1);
          this.componentData.splice(index - 1, 0, component);
          this.curComponentIndex = index - 1;
          this.recordSnapshot(this.$t('message.customDesign.moveDown'));
        } else {
          this.$message.warning(this.$t('message.customDesign.alreadyBottom'));
        }
      } else if (type === 'top') {
        if (index < this.componentData.length - 1) {
          this.componentData.splice(index, 1);
        ...
        if (index > 0) {
          ...
          this.recordSnapshot(this.$t('message.customDesign.moveBottom'));
        } else {
          this.$message.warning(this.$t('message.customDesign.alreadyBottom'));
        }
      }
    ...
    handleAttrChange() {
      this.recordSnapshot(this.$t('message.customDesign.styleSetting'));
    },
    handleUpdateGroup() {
      this.recordSnapshot(this.$t('message.customDesign.positionSetting'));
    },
    save(close = false) {
      const data = {
      ...
      if (close) {
        ...
        this.$message.success(this.$t('message.customDesign.saveSuccess'));
      }
    },
    ...
    handleMenuAction(action) {
      ...
      switch (action) {
        ...
        case 'lock':
          ...
          this.recordSnapshot(this.$t('message.customDesign.lock'));
          break;
        case 'unlock':
          this.$set(this.curComponent, 'isLock', false);
          this.recordSnapshot(this.$t('message.customDesign.unlock'));
          break;
      }
    },
  },
