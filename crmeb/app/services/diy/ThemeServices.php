...
class ThemeServices extends BaseServices
{
    ...
    public function getThemeInfo($id, $type = 'all')
    {
        ...
        if ($type == 'home') {
            ...
            return $this->translateHomeDiyText($homeData);
    ...
    /**
     * Dịch các nhãn tiếng Trung trong cấu hình DIY trang chủ sang tiếng Việt.
     * Chỉ tác động text hiển thị, không đổi URL/path/base64.
     *
     * @param mixed $payload
     * @return mixed
     */
    protected function translateHomeDiyText($payload)
    {
        ...
        if (is_array($payload)) {
            foreach ($payload as $key => $value) {
                $payload[$key] = $this->translateHomeDiyText($value);
            }
            return $payload;
        }
        return $payload;
    ...
    protected function translateHomeDiyString(string $value): string
    {
        ...
        if (strpos($trimmed, 'http://') === 0
            ...
            || strpos($trimmed, '/pages/') === 0
            || strpos($trimmed, '/uploads/') === 0
            || strpos($trimmed, 'data:image') === 0) {
            return $value;
        }
        ...
        return str_replace(array_keys($partialMap), array_values($partialMap), $value);
    }
