...
class SystemStoreStaffServices extends BaseServices
{
    ...
    public function createStoreStaffForm(array $formData = [])
    {
        ...
        $field[] = $this->builder->radio('verify_status', 'công tắc xóa', $formData['verify_status'] ?? 1)->options([['value' => 1, 'label' => 'bật lên'], ['value' => 0, 'label' => 'đóng cửa']]);
        $field[] = $this->builder->radio('status', 'tình trạng', $formData['status'] ?? 1)->options([['value' => 1, 'label' => 'bật lên'], ['value' => 0, 'label' => 'đóng cửa']]);
        return $field;
    }
